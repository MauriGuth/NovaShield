/*
 ...
 Copias:
   - apps/mobile/modules/nova-shield/ios/ShieldBlocklist.swift  (lo compila el pod → la app)
   - apps/mobile/targets/_shared/ShieldBlocklist.swift          (lo compilan las extensiones)

 Las extensiones NO enlazan los pods de Expo, así que no pueden compilar nada
 del módulo. Un symlink tampoco sirve: Xcode lo resuelve y arrastra todo el
 directorio del módulo al target, incluido NovaShieldModule.swift, que importa
 ExpoModulesCore (build roto con "no such module 'ExpoModulesCore'").

 Si tocás una copia, copiala sobre la otra. El test
 apps/backend/src/shield/native-parity.spec.ts las compara byte a byte.
*/
import CryptoKit
import Foundation
