...
- Sin toolchain: `npx expo-modules-autolinking search -p android` / `-p apple` (resuelve módulo y clase) y `npx expo config --type prebuild` (targets registrados).
- **Android SÍ se puede compilar de verdad acá**, y conviene hacerlo antes de mandar un build de EAS: bajar `commandlinetools-linux`, `sdkmanager "platforms;android-36" "build-tools;36.0.0"`, `npx expo prebuild --platform android --no-install` y después `./gradlew :nova-shield:compileReleaseKotlin :app:processReleaseMainManifest` desde `apps/mobile/android`. Eso encuentra los errores de Kotlin y del merge del manifest en minutos en vez de veinte. El APK completo no sale sin NDK (`react-native-screens` compila C++); no hace falta instalarlo. `prebuild` reescribe los scripts `android`/`ios` de `apps/mobile/package.json` a `expo run:*` — revertir ese cambio, el proyecto usa dev client.
- iOS no se compila acá: solo en una Mac.
...
- El podspec del módulo nativo NO puede llamarse igual que la app (`NovaShield`): CocoaPods crea un scheme por cada pod local y `xcodebuild -scheme NovaShield` termina archivando el pod en vez de la app — el archive sale vacío y el export falla con `expected one {}`. Se llama `NovaShieldNative`; `native-parity.spec.ts` lo verifica.
- El módulo Android tiene que declarar `androidx.activity` en su `build.gradle`: expo-modules-core usa `ActivityResultCallback` en su API pública pero declara esa librería como `implementation`, así que no llega transitivamente y el launcher del consentimiento de VPN no compila.
- El `minSdk` efectivo es 24: **nunca `Notification.Builder(Context, String)`** (API 26+), siempre `NotificationCompat`. Revienta con `NoSuchMethodError` solo en Android 7, que es justo el equipo viejo de la persona a la que más queremos proteger.
- Todo perfil de EAS con `distribution: "internal"` tiene que declarar `android.buildType: "apk"`: un `.aab` no se instala en un teléfono y el build sale "bien" igual. `native-parity.spec.ts` lo verifica.
- Bloquear tiene que AVISAR en las dos plataformas. Un bloqueo silencioso se lee como "me quedé sin internet" y termina con el escudo apagado. Throttling parejo: 30 s entre avisos, 10 min por dominio.
- En Android, `start()` vuelve apenas el sistema acepta levantar el `Service`: nada de lo que falle adentro puede volver por esa promesa. Los fallos se escriben en `SharedPreferences` (`ShieldBus.publishError`) y la app los lee por `getShieldDiagnostics().lastError`. Un fallo que no se ve es peor que un fallo.
- `@bacons/apple-targets` deriva el bundle id de cada extensión del **`type`** del target, NO del `name`: `type: 'network-packet-tunnel'` → `<bundleId>.network-packet-tunnel`. El `providerBundleIdentifier` del Swift tiene que coincidir exacto o iOS no encuentra la extensión y el túnel no arranca sin ningún error que lo explique. `native-parity.spec.ts` lo verifica.
